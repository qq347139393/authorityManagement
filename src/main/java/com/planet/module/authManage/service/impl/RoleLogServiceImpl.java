...
import com.planet.module.authManage.dao.mysql.mapper.RoleLogMapper;
...
public class RoleLogServiceImpl extends ServiceImpl<RoleLogMapper, RoleLog> implements RoleLogService {
    @Autowired
    private RoleLogExcelToPoConverter roleLogExcelToPoConverter;
    @Override
    public IPage<RoleLog> selectsByPage(RoleLog t) {
        Page<RoleLog> page = new Page(t.getCurrent(), t.getSize());
        //这里先做出基本的属性遍历查询,以后要加入模糊、大小等各种可能出现的情况的判断方式
        QueryWrapper<RoleLog> tQueryWrapper = new QueryWrapper<>();
...
//        QueryWrapper<aClass> aClassQueryWrapper=new QueryWrapper<aClass>();
        ...
        IPage<RoleLog> pageData = page(page, tQueryWrapper.orderByAsc("id"));
        return pageData;
    }

    @Override
    public void excelExport(RoleLog t) {
        HttpServletResponse response = WebUtil.getResponse();
        // 1.模板
        InputStream templateInputStream = this.getClass().getClassLoader().getResourceAsStream(
                "templates/excel/modular/角色操作记录模块-模板.xlsx");

        // 2.目标文件
        String targetFile = "角色操作记录模块-记录.xlsx";

        //3.模型实体类的类对象
        Class roleLogClass= com.planet.module.authManage.entity.excel.RoleLog.class;

        // 4.写入workbook对象
        ExcelWriter workBook =null;
        try {
            workBook = EasyExcel.write(response.getOutputStream(),roleLogClass).withTemplate(templateInputStream).build();
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        //6.获取数据
        QueryWrapper<com.planet.module.authManage.entity.mysql.RoleLog> tQueryWrapper = new QueryWrapper<>();
        Map<String, Object> stringObjectMap = null;
        try {
            stringObjectMap = MapAndEntityConvertUtil.entityToMap(t, ServiceConstant.CLOSE,true);
        } catch (IllegalAccessException e) {
...
//            IPage<T> pageData = iService.page(page, tQueryWrapper.orderByAsc("id"));
        List<RoleLog> list=list(tQueryWrapper.orderByAsc("id"));
        List<com.planet.module.authManage.entity.excel.RoleLog> roleLogs = roleLogExcelToPoConverter.convertPoToExcel(list);

        //7. 写入数据

        workBook.fill(roleLogs, sheet);
